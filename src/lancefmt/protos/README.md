...
The golden fixtures under `tests/fixtures/lancefmt/` were generated by the
official crate and are exercised by the in-house reader's test suite
(`tests::test_lancefmt_impl`), keeping the "official writer -> our reader"
interop bar green without a runtime or dev dependency on `lance`.
